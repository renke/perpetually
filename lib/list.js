...
class List extends React.Component {
  constructor(props) {
    ...
    let items = this.prepareChildren(this.props.children);
  ...
  prepareChildren(children) {
    let rawItems = this.convertChildren(children);
  ...
  componentWillReceiveProps(newProps) {
    let newItems = this.prepareChildren(newProps.children);
  ...
  update() {
    ...
    itemNodes.forEach((itemNode, i) => {
      ...
      // let oldRect = this.rects.get(this.state.items[itemIndex].key);
      //
      // if (oldRect) {
      //   if (rect.height !== oldRect.height) {
      //     console.log("item changed height");
      //   }
      // }
    ...
    if (this.bottomDemand) {
      // console.log("bottomDemand", this.bottomDemand, "end", this.state.end);
